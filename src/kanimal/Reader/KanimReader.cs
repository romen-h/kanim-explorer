...
namespace kanimal
{
	public class KanimReader : Reader
	{
		...
		public KanimReader(Stream build, Stream anim, Stream img)
		{
			...
			{
				Logger.LogCritical("The given \"img\" stream is not a valid image file. Original exception is as follows:");
		...
		public void ReadBuildData()
		{
			...
			catch (HeaderAssertException ex)
			{
				Logger.LogError(ex, "Invalid header in build.bytes.");
		...
		public void ReadAnimData()
		{
			...
			{
				Logger.LogError(ex, "Invalid header in anim.bytes.");
				Environment.Exit((int)ExitCodes.IncorrectHeader);
			}
		...
		private void ReadAnimIds()
		{
			...
			foreach (var bank in AnimData.Anims)
				foreach (var frame in bank.Frames)
					...
					{
						var name = $"{AnimHashes[element.ImageHash]}_{element.Index}_{element.Layer}";
		...
		{
			Logger.LogInformation("Parsing build data.");
			ReadBuildData();

			Logger.LogInformation("Importing textures.");
			ExportTextures();

			Logger.LogInformation("Parsing animation data.");
			ReadAnimData();
		}
